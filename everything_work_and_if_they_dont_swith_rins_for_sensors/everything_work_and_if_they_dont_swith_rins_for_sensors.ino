 //прерываний не будет. прерывания поддерживают только два порта :(  
  ...
  const int tempSensor35DzOldPin = A2;   
  const int tempSensor35DzNewPin = A1;   
...
void loop() {
  ...
  if (startStat == LOW){
    ...
    if(!keepConstantTemperature && heatingActive == true)
    {
      digitalWrite(heaterPin1, HIGH);
      digitalWrite(heaterPin2, HIGH);
      heatingActive = false;
  ...
  {
    if (currentTemp < targetTemp - hysteresis && heatingActive == false) {
      heatingActive = true;
      digitalWrite(heaterPin1, LOW);
      digitalWrite(heaterPin2, LOW);
    } else if (currentTemp > targetTemp + hysteresis && heatingActive == true) {
      heatingActive = false;
      digitalWrite(heaterPin1, HIGH);
      digitalWrite(heaterPin2, HIGH);
    }  
